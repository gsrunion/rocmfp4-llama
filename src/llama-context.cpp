...
int llama_context::encode(const llama_batch & batch_inp) {
    ...
    // (dflash fuses target_layers x n_embd per row — n_embd_inp_enc(), not n_embd_inp())
    ...
    if (embd_pre_norm.data && t_h_pre_norm && cparams.pooling_type == LLAMA_POOLING_TYPE_NONE) {
        ...
        // width from the tensor, not hparams (see decode-path note re dflash)
        const uint32_t n_embd = (uint32_t) t_h_pre_norm->ne[0];
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        if (embd_pre_norm.data && t_h_pre_norm && n_outputs > 0 && cparams.pooling_type == LLAMA_POOLING_TYPE_NONE) {
            ...
            // row width comes from the graph tensor, not hparams: dflash
            // drafts expose a narrow (draft-width) hidden while n_embd_out()
            // reflects the borrowed target-width output head. For all other
            // pre_norm producers (eagle3/mtp) ne[0] == n_embd_out() anyway.
            const uint32_t n_embd = (uint32_t) t_h_pre_norm->ne[0];
            if (n_embd != hparams.n_embd_out()) {
                LLAMA_LOG_WARN("%s: pre_norm width %u != n_embd_out %u — tensor '%s' [%lld,%lld] arch=%s n_rows=%lld masked=%d\n",
                    __func__, n_embd, hparams.n_embd_out(), t_h_pre_norm->name,
                    (long long) t_h_pre_norm->ne[0], (long long) t_h_pre_norm->ne[1],
                    llm_arch_name(model.arch), (long long) n_rows, (int) masked);
            }
