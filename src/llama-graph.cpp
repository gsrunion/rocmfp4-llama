...
void llm_graph_result::reset() {
    ...
    // latent bug fixed during the DFlash graft: this slot was never reset,
    // leaving a dangling pointer whenever a graph type that doesn't set it
    // (e.g. dflash inject/diffusion) followed one that does (encoder) —
    // downstream consumers then read recycled tensor memory (seen live as
    // attn_inp_kq_mask bytes fed to the draft as "target features").
